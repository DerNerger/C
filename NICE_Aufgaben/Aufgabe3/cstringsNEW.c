...
enum Zustand {outside_str_or_char, inside_str, escape_inside_str, inside_char, escape_inside_char};

int main( void ) {
  int c;
  int zustand = outside_str_or_char;
  ...
  while ( (c = getchar()) != EOF ) {
    ...
    case outside_str_or_char:            /* außerhalb von Zeichen- und Stringkonstanten         */   
      ...
      case inside_char:       /* innerhalb einer Zeichenkonstante, jedoch nicht nach \  */ 
             switch ( c ) {
               case '\\':  zustand = escape_inside_char;
                           break;
